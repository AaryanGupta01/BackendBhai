...
const getSqlPath = (sub) => {
  const p1 = path.resolve(__dirname, `../db/${sub}/001_initial.sql`);
  if (fs.existsSync(p1)) return p1;
  return path.resolve(__dirname, `../infrastructure/db/${sub}/001_initial.sql`);
};

const devtoolsSqlPath = getSqlPath('devtools');
const ecommerceSqlPath = getSqlPath('ecommerce');
