 ...
 */
function initTracing(defaultServiceName = 'unknown-service') {
    if (sdkInstance) {
    ...
    try {
        if (otlpEndpoint.includes(':4317')) {
            ...
            traceExporter = new exporter_trace_otlp_http_1.OTLPTraceExporter({ url: otlpEndpoint });
        }
    }
    catch {
        traceExporter = new exporter_trace_otlp_http_1.OTLPTraceExporter({ url: otlpEndpoint });
    ...
    sdkInstance = new sdk_node_1.NodeSDK({
        resource,
        traceExporter,
        instrumentations: [
